...
const weaponsData = JSON.parse(fs.readFileSync('propulsion.json', 'utf8'));
...
function formatWeaponData (id, body) {
    return '' +
        ...
        (body.deceleration?.toString() || '').padEnd(20) +
        (body.id?.toString() || '').padEnd(20) +
        (body.buildPoints?.toString() || '').padEnd(20) +
        (body.buildPower?.toString() || '').padEnd(20) +
        (body.speed?.toString() || '').padEnd(20) +
        (body.spinAngle?.toString() || '').padEnd(20) +
        (body.spinSpeed?.toString() || '').padEnd(20) +
        (body.turnSpeed?.toString() || '').padEnd(20) +
        (body.type?.toString() || '').padEnd(20) +
...
fs.writeFileSync('prop.txt', outputData);
